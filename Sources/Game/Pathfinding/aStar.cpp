...
bool AStarPathfinder::isPositionValid(const Vector2& pos, const std::vector<std::vector<int>>& grid) {
    if (pos.x < 0 || pos.x >= static_cast<int>(grid.size()) || 
        pos.y < 0 || pos.y >= static_cast<int>(grid[0].size())) {
        return false;
    }
...
std::vector<Vector2> AStarPathfinder::findPath(
    ...
    while (!openList.empty()) {
        ...
        if (closed[current->position.x][current->position.y]) {
            continue;
        }
        closed[current->position.x][current->position.y] = true;

        for (const auto& dir : directions) {
            Vector2 neighborPos = { current->position.x + dir.x, current->position.y + dir.y };

            if (isPositionValid(neighborPos, grid) && !closed[neighborPos.x][neighborPos.y]) {
